...
namespace YUCP.DevTools.Editor.PackageExporter
{
    ...
    public static class ConfuserExManager
    {
        ...
        public static string GenerateProjectFile(
            ...
            ObfuscationSettings settings = null)
        {
            ...
            // Add Unity exclusion patterns if reflection preservation is enabled
            if (settings != null && settings.preserveReflection)
            {
                string exclusionPatterns = ConfuserExPresetGenerator.GenerateUnityExclusionPatterns(settings);
                if (!string.IsNullOrEmpty(exclusionPatterns))
                {
                    sb.AppendLine(exclusionPatterns);
                }
            }
            
            sb.AppendLine("  <rule pattern=\"true\" inherit=\"false\">");
            
            // Add protection rules using preset and settings
            string protectionRules = ConfuserExPresetGenerator.GenerateProtectionRules(preset, settings);
            ...
            ObfuscationSettings settings = null)
        {
            ...
            try
            {
                ...
                string projectFilePath = GenerateProjectFile(validAssemblies, preset, workingDir, settings);
